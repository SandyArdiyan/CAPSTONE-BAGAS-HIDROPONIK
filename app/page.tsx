          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100 relative overflow-hidden">
              ...
              <div className="text-6xl font-black text-gray-800 mb-4">
                {tdsData.length > 0 ? tdsData[0].nilai_tds : 0} <span className="text-2xl text-gray-400 font-medium ml-2">ppm</span>
              </div>
            </div>

            <div className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100">
              <h3 className="text-gray-400 text-sm font-bold uppercase tracking-widest mb-2">Status Pompa Nutrisi</h3>
              <div className={`text-5xl font-black mb-4 ${pumpLogs.length > 0 && pumpLogs[0].status_pompa === 'ON' ? 'text-green-500' : 'text-rose-500'}`}>
                {pumpLogs.length > 0 ? pumpLogs[0].status_pompa : "OFF"}
              </div>
            </div>
          </div>
        )}
